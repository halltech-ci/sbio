...
class ReportTimeSheetReportView(models.AbstractModel):
    ...
    """
    _name = 'report.hta_stock.report_pos_sale'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        filter_pdv = data['form']['filter_pdv']
        product_id = data['form']['product_id']
        docs = []
        
        if product_id:
            pos_list = self.env['pos.order.line'].search_read([('name','ilike',filter_pdv),('product_id','in',product_id),('create_date','>=',date_start),('create_date','<=',date_end),('order_id.state','!=','return'),('order_id.audit','=','valide'),('full_product_name','not ilike','réduction')])
        else:
            pos_list = self.env['pos.order.line'].search_read([('name','ilike',filter_pdv),('create_date','>=',date_start),('create_date','<=',date_end),('full_product_name','not ilike','Livraison'),('full_product_name','not ilike','Remise'),('full_product_name','not ilike','réduction'),('order_id.state','!=','return'),('order_id.audit','=','valide')])

        # Regrouper par produit
        pos_list_sorted = sorted(pos_list, key=itemgetter('product_id'))

        for product, lines in groupby(pos_list_sorted, key=itemgetter('product_id')):
        ...
        return {
            'doc_model': 'report.pos.wizard',
            'date_start': date_start,
            'date_end': date_end,
            'docs': docs,
            'filter_pdv':filter_pdv,
        }
